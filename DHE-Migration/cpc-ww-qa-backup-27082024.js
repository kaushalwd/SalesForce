                      ...
                      %%[
                                ...
                                IF RequestParameter("submittedUnsub")==true then
                                   ...
                                   SET @updateRecord = UpdateSingleSalesforceObject(
                                    ...
                                    IF NOT Empty(@unsubTempBox) THEN
                                     SET @s = upsertData("ENT.TempPauseHandleDPR_QA", 1, "SubscriberKey", @guestId, "Email",
                                                            @emailContact,"DateAdded",@currentDate,"ContactId",@sfid, "UnsubscribeType","Individual","Asset","WW")
                                    ENDIF
                                    IF Empty(@unsubTempBox) THEN
                                    set @rowFound = LookupRows("ENT.TempPauseHandleDPR_QA","SubscriberKey", @guestId)
                                    set @count = rowcount(@rowFound)
                                    if @count > 0 then
                                    set @deleteCount = DeleteData("ENT.TempPauseHandleDPR_QA","Email", @emailContact)
                                    SET @updateRecord = UpdateSingleSalesforceObject(
                                    "Guest_Subscription__c", @guestId,
                                    "Temporary_Pause_30_Days__c", IIF(Empty(@unsubTempBox), 'False', 'True'), 
                                    "Emails_are_too_frequent__c", IIF(Empty(@unsubFreqBox), 'False', 'True'),
                      ...
                      %%[
                                ...
                                IF NOT Empty(@unsubTempBox) THEN
                                   /* Subscribe from all guest subscriptions */
                                    ...
                                    SET @s = upsertData("ENT.TempPauseHandleDPR_QA", 1, "SubscriberKey", @guestId, "Email", @emailContact,"DateAdded",@currentDate,"ContactId",@sfid,"UnsubscribeType","All","Asset","WW")
                                    
                                    /*Upsert data in resub DE to resubscriber contact on enterprise level*/
                                    
                                    SET @resub = upsertData("ENT.UnsubscribeHandleDPR_QA", 1, "SubscriberKey", @sfid, "EmailAddress", @emailContact, "SubsciberStatus", "Active","Temp_30_Days", "True","Frequency","False","Relevance","False","Not_In_Dubai","False","Other","False","Asset","WW")
W                                    
