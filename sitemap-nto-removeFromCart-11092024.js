          ...
          pageTypes: [
              {
                  ...
                  */
                  ...
                  listeners: [
                      ...
                      SalesforceInteractions.listener("click", ".remove-product", () => {
                        let removeItem = { catalogObjectType: "Product" };
                        removeItem.catalogObjectId = window.dataLayer.filter((x) => x.event == "removeToCart")[(window.dataLayer.filter((x) => x.event == "removeToCart").length) - 1].ecommerce.remove.products[0].id;
                        removeItem.quantity = 1;
                        removeItem.price = parseFloat(window.dataLayer.filter((x) => x.event == "removeToCart")[(window.dataLayer.filter((x) => x.event == "removeToCart").length) - 1].ecommerce.remove.products[0].price.replace(/[^0-9\.]+/g, ""));
                        SalesforceInteractions.sendEvent({
                            interaction: {
